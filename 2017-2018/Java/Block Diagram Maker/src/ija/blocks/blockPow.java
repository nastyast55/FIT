...
public class blockPow extends AbstractBlock{
    private connection input2;

    /* While user creates blocks */
    public blockPow(String type, int connID){
        this.output = new connection(type, connID);
    }

    public blockPow(String type, connection c1, connection c2, int connID)
    {
    	super.input1=c1;
        this.input2=c2;
    ...
    public void execute() {
        output.setValue(Math.pow(input1.getValue(),input2.getValue()));
    }
}